		...
		viewModel.apply {
			searchData.observe {
				...
				questionCardAdapter.submitList(
					listOf(QuestionCardUI.Header(viewModel.getCurrentQuestion())) + it.map(QuestionCardUI::QuestionItem)
				) {
					mRvSearch.scrollToPosition(0)
				}
	...
	private fun initView() {
		...
		questionCardAdapter.apply {
			...
			setOnTitleClickListener {
				...
				mRvSearch.gone()
				mEditText.visible()
				tagSelectorAdapter.requestTagClickable(true)
				mEditText.startInput()
				viewModel.setIsSearchPage(false)
			}
		}
