...
fun Modifier.longPressMove(
  ...
  return layout { measure, constraints ->
    ...
    if (controllerWrapper.state is LongPressMoveState.Touching) {
      ...
      if (changeTransition != transition.value) {
    ...
    awaitEachGesture {
      ...
      while (true) {
        ...
        if (pointer == null || pointer.isConsumed || pointer.changedToUp()) {
          val isChangedToUp = pointer?.changedToUp() ?: false
          if (controllerWrapper.state is LongPressMoveState.Touching) {
...
class LongPressMoveControllerImpl(
  ...
  override fun onMoveLongPress(
    ...
    super.onMoveLongPress(nowTransition, changeTransition, isTouch, screenLeftTop, size)
    if (isTouch) {
      // 尝试触发边缘滚动
      itemState.coursePageFlow.value?.scrollContext?.edgeScroll?.tryEdgeScroll(
        key = "LongPressMoveControllerImpl_$itemState",
        screenTopY = screenLeftTop.y,
        height = size.height,
      )
    }
  ...
  override suspend fun animateMove(
    ...
    if (destinationOffset == Offset.Zero) {
      ...
      ): MutableSet<CourseItemState> {
        itemState.overlap?.coveredRangeList?.fastForEach { cover ->
          set.add(cover.result.itemState)
          collectTopItems(set, cover.result.itemState)
        }
        return set
      }
      topItemStateList = collectTopItems(mutableSetOf(), itemState).onEach {
