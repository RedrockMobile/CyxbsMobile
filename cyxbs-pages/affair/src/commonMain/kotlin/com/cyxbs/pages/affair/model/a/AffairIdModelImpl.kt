...
class AffairIdModelImpl(
  ...
  private class Data(stuNum: String, localId: String) {
    val settings = PreferencesSettings.get("affair_${stuNum}_${localId}")

    val remoteIdState: StateFlow<Int> = MutableStateFlow(settings.getInt("remoteId", 0))
    var remoteId: Int
      get() = remoteIdState.value
      set(value) {
        remoteIdState.value = value
        settings.putInt("remoteId", value)
      }

    val remindTime: Int by settings.int("remindTime", 0)
    val title: String,
    val content: String,
  }
}