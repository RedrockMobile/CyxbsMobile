...
internal class NpmIntegrity private constructor(
  ...
  internal companion object {
     ...
     * 解析 npm 的 SRI 文本。
     *
     * 当前只接受单个 SHA-256 或 SHA-512 token，避免多摘要降级和 SHA-1。后端应优先下发 npm
     * registry 的 SHA-512 integrity。
     */
    ...
    fun parse(value: String, packageName: String): NpmIntegrity {
      ...
      if (token.isEmpty() || token.any(Char::isWhitespace)) {
        throw NpmSnapshotException("Package '$packageName' integrity must contain one SRI token.")
      }
      val separator = token.indexOf('-')
      if (separator <= 0 || separator == token.lastIndex) {
        throw NpmSnapshotException("Package '$packageName' integrity is not a valid SRI token.")
      }
      val algorithm = when (token.substring(0, separator)) {
        "sha256" -> Algorithm.SHA256
        "sha512" -> Algorithm.SHA512
        else -> throw NpmSnapshotException(
          "Package '$packageName' integrity must use SHA-256 or SHA-512.",
        )
      }
      val digest = token.substring(separator + 1).decodeBase64()
        ?: throw NpmSnapshotException("Package '$packageName' integrity digest is not Base64.")
