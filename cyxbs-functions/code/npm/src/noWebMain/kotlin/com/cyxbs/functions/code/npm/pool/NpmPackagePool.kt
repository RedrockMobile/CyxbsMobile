package com.cyxbs.functions.code.npm.pool
...
import com.cyxbs.functions.code.npm.internal.NpmRegistryPackageMetadata
 ...
 * 6. 同一实际 [rootDirectory] 在一个进程内只能由一个长生命周期实例持有；多个 Pool 的运行
 *    租约、刷新记录和 Mutex 不会相互协调，共享目录可能造成状态覆盖或误删正在使用的归档。
 * 7. 不支持多个进程共享同一目录；自定义 Pool 必须使用区别于 [Default] 和其他 Pool 的独立目录。
 *
...
class NpmPackagePool(
   ...
   * 获取一个完整入口并持有运行租约。
   *
   * 调用方至少应在 [com.cyxbs.functions.code.npm.module.NpmModuleGraphFactory.create] 完成前持有租约；如果运行期仍会回读归档，则应持有到
  ...
  companion object {
     ...
     * 默认配置的进程级全局包池。
     *
     * 所有默认 [com.cyxbs.functions.code.npm.NpmJsServiceLoader] 共享该实例，使 Latest 刷新、运行
     * 租约和 GC 状态保持一致。自定义目录、registry 或 Transport 时应显式创建独立实例，并使用
     * 不与本实例或其他 Pool 重合的缓存目录。
     */
    val Default: NpmPackagePool by lazy { NpmPackagePool() }
