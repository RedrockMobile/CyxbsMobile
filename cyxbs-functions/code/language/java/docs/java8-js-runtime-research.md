...
> 反射、线程、文件、网络、动态加载和完整 Java SE 仍不在支持范围，也尚未作为完整 Java 8
> 产品发布；线程虽完成 Desktop 可行性原型，但已决定暂缓生产实现。
>
...
| 受限真并行线程扩展（已延期） | 经典共享计数、自增竞态、`synchronized` 和独立计算加速 | 在所选档位上增加 4k～8k | 增加 3k～6k | 增加约 0.2～0.8 MiB；运行内存按线程增加 Runtime | 额外约 1.5～3 人月；当前不投入生产实现 |
...
### 5.5 受限真并行线程方案（Desktop 原型完成，生产实现已延期）

线程能力采用真正的宿主多线程，而不是单 Runtime 交错执行。该方案用于教学中的共享计数、
未同步自增丢失、`synchronized` 修复竞态，以及独立 CPU 任务并行后耗时下降。Desktop 技术
原型已经通过三项闸门，但生产 Java API、共享对象桥、Android 与 iOS 验证尚未完成。经成本评估，
当前决定暂缓该能力，不对业务开放，也不把它列入近期工程批次。
...
这里最主要的成本不是创建宿主线程，而是跨 Runtime 维持 Java 共享对象语义。普通 `JSValue` 不能
跨 Runtime 使用；如果把对象复制给子线程，又会破坏 Java 的共享引用、对象身份和未同步竞态。
因此正式实现必须先建设 SharedHeap/Handle ABI，或以 SharedArrayBuffer/原生连续内存承载高频
基础字段，并同时覆盖引用写屏障、monitor、happens-before、取消和资源回收。quickjs-kt 的原生
上下文访问只能优化这座桥，不能消除它。完整冻结边界与重新开启条件见
[java-thread-runtime-prototype.md](java-thread-runtime-prototype.md)。
...
| 受限真并行 `Thread` 与 `synchronized` | 中 | 很高 | 4k～8k | 0.2～0.8 MiB；每线程额外持有 Runtime 内存 | 每线程独立 Runtime；基础字段、静态字段和基础数组下沉宿主共享堆；对象句柄 monitor | Desktop 原型已完成；生产实现明确延期，当前保持不支持 |
...
- （Desktop 原型已完成，生产已延期，2026-08-18）受限真并行线程原型通过丢失更新、monitor
  修复和双 Runtime CPU 加速三项闸门；由于共享对象桥、跨平台资源治理和性能成本较高，当前
  不实现生产 Thread API，不启动 Android/iOS 产品闸门；
...
5. **线程共享模型**：受限真并行要求每线程独立 Runtime，并把共享字段、数组和 monitor 下沉宿主；
   原生上下文访问不能消除跨 Runtime 共享桥。由于桥接、内存和完整 JMM 成本较高，生产线程能力
   当前明确延期，Desktop 原型只保留为未来重新评估的证据。
